...
# Check if the module is loaded (at least one device or the module name in lsmod):
is_module_loaded() {
...
load_module() {
  ...
  #   echo "${FMT_YELLOW}Removing $MODULE_NAME module to re-insert with updated devices...${FMT_RESET}"
  #   if ! modprobe -r "$MODULE_NAME"; then
  #     echo "${FMT_RED}Failed to remove $MODULE_NAME module.${FMT_RESET}"
  #     exit 2
  #   fi
  # fi
  unload_module
  ...
  if is_module_loaded; then
    ...
    if [ "$assigned_dev" = "-1" ]; then
      ...
      echo "${FMT_GREEN}v4l2loopback loaded successfully with new device /dev/video${assigned_dev} (label: ${V4L_LABEL}-${NEXT_FREE})${FMT_RESET}"
    fi
  else
...
unload_module() {
  ...
  LOADED="$(get_loaded_devices)"
  if [ -z "$LOADED" ]; then
    echo "${FMT_YELLOW}No $V4L_LABEL devices are currently loaded.${FMT_RESET}"
    return 0
  fi
  ...
  if [ -z "$INUSE_DEVICES" ]; then
    ...
    if ! is_module_loaded; then
      echo "${FMT_GREEN}All $V4L_LABEL devices removed successfully.${FMT_RESET}"
    else
      echo "${FMT_RED}Failed to unload $MODULE_NAME module.${FMT_RESET}"
      exit 3
    fi
...
check_module() {
  if ! is_module_loaded; then
    echo "${FMT_YELLOW}No $MODULE_NAME module loaded.${FMT_RESET}"
...
# Start streaming:
echo "${FMT_BLUE}Starting video stream from IP camera at $CAMERA_IP to ${VIDEO_DEVICE}. Press Ctrl+C to stop.${FMT_RESET}"
